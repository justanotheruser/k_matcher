...
def test_post_answers__unreleated_users(test_client: TestClient, fill_db_with_questions):
    ...
    first_response = test_client.post("/result", json=test_data)
    ...
    second_response = test_client.post("/result", json=test_data)
...
def test_post_answers__related_users(test_client: TestClient, fill_db_with_questions):
    ...
    first_result_id = first_response_json["id"]
    assert first_result_id is not None
    # Second user sends answers (id from repsponse to first user)
    test_data = {
    ...
    for graded_match in second_response_json["matching_result"]:
        graded_match["matches"].sort(key=lambda q: q["question_id"])
    assert second_response_json["matching_result"] == [
        {
            "min_answer": 3,
            "matches": [
                {
    ...
    # TODO
    # test_client.get(f"/result/{first_result_id}")
